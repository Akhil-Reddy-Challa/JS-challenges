...
checkBoxes.forEach((cbox, idx) => {
  cbox.addEventListener("click", function (e) {
    console.log(`Clicked check box ${idx}`);
    if (e.shiftKey) handleMultiple(idx);
    else userSelected = idx;
  });
});

function handleMultiple(idx) {
  console.log(`Mark down everything from ${userSelected} till ${idx}`);
  for (
    let i = Math.min(idx, userSelected);
    i <= Math.max(idx, userSelected);
    i++
  ) {
